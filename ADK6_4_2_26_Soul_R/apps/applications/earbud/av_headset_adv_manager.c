...
#ifdef SOUL_BLADE
#define BT_SIG_ID_BEFLEX        0x6B9
#define HW_MODEL_BLADE_L        254 
#define HW_MODEL_BLADE_R        253     // SOUL Blade (L) : 254 , SOUL Blade (R) : 253 , Beflex (L) : 252 , Beflex (R) : 251
#define HW_MODEL_BEFLEX_L       252 
#define HW_MODEL_BEFLEX_R       251 
#define HW_VER          1       // device version
#define FW_HOST_VER     1       // nRF52832 firmware version
#define FW_BME_VER      1       // BME-300 firmware version

typedef struct _ble_adv_manufacturer_info {
    uint16 id;
    uint8 hwModel;
    uint8 hwVer;
    uint8 hostFwVer;
...
static uint8* appAdvManagerAdvertdataAddManufacturer(uint8 *ad_data, uint8* space, uint8 *data)
{
    uint8 field_length;
    ...
    if (usable_space < 6)
    {
        DEBUG_LOG("appAdvManagerAdvertdataAddManufacturer. No space for Manufacturer in advert");
    }
    else
    {
        field_length = AD_FIELD_LENGTH(MIN(usable_space,6));
        ad_data = addHeaderToAdData(ad_data, space, field_length, ad_tag);
        memmove(ad_data, data, field_length);
        ad_data += field_length;
        *space -= field_length;
    }
...
static void appAdvManagerSetupAdvert(advManagerAdvert *advert)
{
    ...
    if (advert->content.local_name)
    {
        ...
        name_len = MIN(MAX_NAME_LEN, strlen((char *)advert->local_name));
#else
    ...
    if (advert->content.local_name)
    {
...
#ifdef SOUL_BLADE
        {
            ...
            if(strcmp((char*)advert->local_name,(char*)"SOUL Blade (L)") == 0)
                hwModel = HW_MODEL_BLADE_L;
            else if(strcmp((char*)advert->local_name,(char*)"SOUL Blade (R)") == 0)
                hwModel = HW_MODEL_BLADE_R;
            else if(strcmp((char*)advert->local_name,(char*)"Beflex (L)") == 0) 
                hwModel = HW_MODEL_BEFLEX_L;
            else if(strcmp((char*)advert->local_name,(char*)"Beflex (R)") == 0) 
                hwModel = HW_MODEL_BEFLEX_R;
            
            const uint8 bleDevInfo[6] = { BT_SIG_ID_BEFLEX&0xFF, BT_SIG_ID_BEFLEX>>8, hwModel, HW_VER, FW_HOST_VER, FW_BME_VER};
    
            ad_head = appAdvManagerAdvertdataAddManufacturer(ad_head, &space, (uint8*)bleDevInfo);
        }
#endif
