...
#ifdef Jim
bool appDeviceGetHandsetAddr(bdaddr *bd_addr)
{
...
bool appDeviceDelete(const bdaddr *bd_addr)
{
    if (!appConManagerIsConnected(bd_addr))
    {
        ...
        theDevice->handset_paired = FALSE; //SOUL_BLADE
...
#ifdef SOUL_BLADE
        case CL_DM_READ_STORED_LINK_KEY_CFM:
        {
            const CL_DM_READ_STORED_LINK_KEY_CFM_T *cfm = message;
            DEBUG_LOG("MINI ]]] READ_STORED_LINK_KEY, status %d, max 0x%x, key 0x%x",cfm->status, cfm->max_num_keys, cfm->num_keys_read);
        }
            break;
#endif            
        default:
