...
#ifdef SOUL_BLADE
    RULE(RULE_EVENT_IN_EAR,                     ruleInEarLedsDisable,               CONN_RULES_LED_ENABLE), 
#else
...
#ifdef SOUL_BLADE    
    RULE(RULE_EVENT_PEER_IN_CASE,               ruleAutoHandsetPair,                CONN_RULES_HANDSET_PAIR),    
#endif    
...
#ifdef SOUL_BLADE       
    RULE(RULE_EVENT_PEER_POWEROFF,              rulePeerSync,                       CONN_RULES_SEND_PEER_SYNC),  
    RULE(RULE_EVENT_PEER_POWEROFF,              rulePeerPowerOff,                   CONN_RULES_PEER_POWEROFF),
#endif    
};
...
#ifdef SOUL_BLADE       
static ruleAction rulePeerPowerOff(void)
{
    if (appSmIsOutOfCase())
    {
        RULE_LOG("rulePeerPowerOff, run");
        return RULE_ACTION_RUN;
    }
    else
    {
        RULE_LOG("rulePeerPowerOff, ignore");    
        return RULE_ACTION_IGNORE;    
    }
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                return RULE_ACTION_IGNORE;//RULE_ACTION_RUN
            }
        }
    }
}
...
#ifdef SOUL_BLADE        
        if (appSmIsConnectable())
        {
            RULE_LOG("ruleCheckUpgradable, allow as appSmIsConnectable");
            return RULE_ACTION_RUN_PARAM(allow_dfu);
        }
        if (appSmIsBleAdvertising())
        {
            RULE_LOG("ruleCheckUpgradable, allow as BLE Advertising");
            return RULE_ACTION_RUN_PARAM(allow_dfu);
        }
#endif
...
static ruleAction ruleBleConnectionUpdate(void)
{
    ...
    connectable = paired_with_peer && ble_connectable;
