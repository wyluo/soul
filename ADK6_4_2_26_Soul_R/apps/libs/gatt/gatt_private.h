...
#define GATT_DEBUG(x)       {printf x;  /*Panic();*/} // SOUL_BLADE
#else
...
enum
{
    ...
    GATT_INTERNAL_SET_BREDR_LOCAL_MTU_REQ,
...
typedef struct
{
    Task    theAppTask;
    uint16  mtu;
}GATT_INTERNAL_SET_BREDR_LOCAL_MTU_REQ_T;
...
void gattHandleInternalSetBrEdrLocalMtuReq(
    const GATT_INTERNAL_SET_BREDR_LOCAL_MTU_REQ_T *m);
void gattHandleAttSetBrEdrLocalMtuCfm(
    const ATT_SET_BREDR_LOCAL_MTU_CFM_T *m);
