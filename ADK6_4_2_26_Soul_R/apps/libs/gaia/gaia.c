...
static void upgrade_disconnect(gaia_transport *transport)
{
    if (gaia->upgrade_transport == transport)
    {
        ...
        #ifdef SOUL_BLADE
        SourceEmpty(StreamSourceFromSink(gaiaTransportGetSink(transport)));  
        StreamConnect(StreamSourceFromSink(gaiaTransportGetSink(transport)), 0);
        StreamDisconnect(StreamSourceFromSink(gaiaTransportGetSink(transport)), 0);
        StreamConnectDispose(StreamSourceFromSink(gaiaTransportGetSink(transport)));
        #endif
