...
//add by kimny
#define APP_MFB_BUTTON_DOUBLE_TIMEOUT	700 //(MSEC)
...
//add by kimny jim
bool press_once_flag = FALSE;

uint8 release1_flag = 0;	//add by kimny jim

uint8 double1_flag = 0;	//add by kimny  jim

uint8 double_timeout_flag = 0;	//add by kimny jim
    ...
    if (input_action->bits == (input_event_bits & input_action->mask))
    {
        ...
        if (state->double_tap == input_action)
        {
            ...
            if (MessageCancelAll(&state->task, IEM_INTERNAL_DOUBLE_TIMER))
            {
				...
				//add by kimny jim
				IEM_DEBUG((">> doubleAction IEM: Updated client %p, input message %d\n", &state->client, input_action->message));

              	double1_flag = 1;	//add by kimny   jim
               	double_timeout_flag = 0;	//add by kimny  jim
            }
        ...
        {
                //add by kimny  jim
        	IEM_DEBUG((">> doubleAction IEM: Updated input action %08x\n", input_action->action));
...
static void inputEventsChanged(InputEventState_t *state, input_event_bits_t input_event_bits)
{
    ...
    {
                //add by kimny jim
		IEM_DEBUG((">> inputEventsChanged IEM: Updated changed_bits %08x, input_action->mask %08x,  input_action->action %08x\n", changed_bits, input_action->mask, input_action->action));
            ...
            switch (input_action->action)
            {
                ...
                case RELEASE:
                {
                    ...
                    {    
                    	...
                    	IEM_DEBUG((">> inputEventsChanged IEM: Sending state->client %p, input_action->message %d\n", state->client, input_action->message));

                        //add by kimny  jim
                        if(input_action->message == APP_MFB_BUTTON_PRESS)	//APP_MFB_BUTTON_PRESS
                        {
                            MessageSend(state->client, APP_MFB_BUTTON_PRESS, 0);
...
static void pioHandler(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case IEM_INTERNAL_DOUBLE_TIMER:
        {
			...
			double_timeout_flag = 1;	//add by kimny  jim
			
			//add by kimny for debug    jim
			IEM_DEBUG(("IEM_INTERNAL_DOUBLE_TIMER: task %p, id %d, message %x  \n", &state->task, id, message));

			#if 0
			if(double1_flag==1)
			{
				double1_flag=0;
				MessageSend(state->client, APP_MFB_BUTTON_DOUBLE, 0);	//APP_MFB_BUTTON_DOUBLE
			}
			else if(release1_flag==1 && double1_flag==0)
			{
				release1_flag=0;
				//MessageSend(state->client, input_action->message, 0);
				MessageSend(state->client, APP_MFB_BUTTON_PRESS, 0);	//APP_MFB_BUTTON_PRESS
			}
			#endif
        }	
            break;

		case IEM_INTERNAL_DOUBLE1_TIMER:
        {
            /* Nothing done here, see the doubleAction() function */
			...
			press_once_flag=FALSE;
			
   			//add by kimny for debug    jim
			IEM_DEBUG(("IEM_INTERNAL_DOUBLE1_TIMER: client %p, task %p, id %d, message %x  \n", &state->client, &state->task, id, message));
