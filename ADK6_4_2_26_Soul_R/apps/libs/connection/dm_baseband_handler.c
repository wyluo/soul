...
#ifdef SOUL_BLADE
void connectionHandleReadStoredLinkKeyReq(const CL_INTERNAL_DM_READ_STORED_LINK_KEY_REQ_T *req)
{
    MAKE_PRIM_C(DM_HCI_READ_STORED_LINK_KEY_REQ); 
    BdaddrConvertVmToBluestack(&prim->bd_addr, &req->bd_addr);
    prim->read_all = (read_all_flag_t)req->read_all;
    VmSendDmPrim(prim);
}

void connectionHandleReadStoredLinkKeyCfm(Task theAppTask, const DM_HCI_READ_STORED_LINK_KEY_CFM_T *cfm)
{
    MAKE_CL_MESSAGE(CL_DM_READ_STORED_LINK_KEY_CFM);
    message->status = connectionConvertHciStatus(cfm->status);
    
    message->max_num_keys = cfm->max_num_keys;
    message->num_keys_read = cfm->num_keys_read;
    MessageSend(theAppTask, CL_DM_READ_STORED_LINK_KEY_CFM, message);
}
