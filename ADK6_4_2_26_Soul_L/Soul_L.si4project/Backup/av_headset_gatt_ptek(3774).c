...
static bool ptekStopCmd(bitserial_handle handle)
{
    ...
    if (BITSERIAL_RESULT_SUCCESS == getReadOffset(handle,value,(uint16)read_size)){
        if(read_size>=2)
            DEBUG_LOGF("Stop RxData[0] = 0x%x 0x%x", value[0], value[1]);
        if(read_size>=4)
            DEBUG_LOGF("Stop RxData[2] = 0x%x 0x%x", value[2], value[3]);
    }
    //free(value);
...
static bool ptekSetCmd(bitserial_handle handle)
{
    ...
    if (BITSERIAL_RESULT_SUCCESS == getReadOffset(handle,value,(uint16)read_size)){
        if(read_size>=2)
            DEBUG_LOGF("SetCmd RxData[0] = 0x%x 0x%x", value[0], value[1]);
        if(read_size>=4)
            DEBUG_LOGF("SetCmd RxData[2] = 0x%x 0x%x", value[2], value[3]);
    }
    //free(value);
...
static bool ptekGetCmd(bitserial_handle handle, uint8 command, uint8* value, uint8* read_size)
{
    ...
    *read_size = getReadSize(handle);
    ...
    if (BITSERIAL_RESULT_SUCCESS == getReadOffset(handle,value,(uint16)*read_size)){
        if(*read_size>=2)
            DEBUG_LOGF("ptekGetCmd CMD 0x%X RxData[0] = 0x%x 0x%x",command, value[0], value[1]);
        if(*read_size>=4)
            DEBUG_LOGF("ptekGetCmd CMD 0x%X RxData[2] = 0x%x 0x%x",command, value[2], value[3]);
        if(*read_size >=6)
            DEBUG_LOGF("ptekGetCmd CMD 0x%X RxData[4] = 0x%x 0x%x",command, value[4], value[5]);
