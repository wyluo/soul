...
static bool ReadOptical(void)//static void ReadOptical(void)
{
    ...
    if(ptekGetCmd(gPtekHandle,(uint8)0x12,(uint8*)value,&read_size))
    {
        DEBUG_LOGF("checkPtekIdleMode read_size= %d",read_size);
...
static bool ReadOffSkin_IR(void)//static void ReadOptical(void)
{
    ...
    if(ptekGetCmd(gPtekHandle,(uint8)0x11,(uint8*)value,&read_size))
    {
        DEBUG_LOGF("checkPtekIdleMode read_size= %d",read_size);
...
{
    Pre_Play = ReadOptical()||ReadOffSkin_IR();
    if(IR_play != Pre_Play)
    {
