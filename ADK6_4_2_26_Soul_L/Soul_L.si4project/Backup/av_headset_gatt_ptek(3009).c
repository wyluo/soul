...
static int getReadSize(bitserial_handle handle)
{
    ...
    int i, read_cnt=0, read_size = 0;
        ...
        //uint8 offset_rsp_len[2] = {PTEK_I2C_WRITE_ADDR, PTEK_RSP_LENGTH_OFFSET};
    #else
        uint8 offset_rsp_len = PTEK_RSP_LENGTH_OFFSET;
    #endif
    uint8 value[2] = {0,0};
    ...
    for(i = 0;i <READ_MAX_CNT;i++)
    {
        ...
        result = BitserialWrite(handle,
                                BITSERIAL_NO_MSG,
                                &offset_rsp_len, sizeof(offset_rsp_len),
                                /*offset_rsp_len, sizeof(offset_rsp_len),*/
        ...
        if (result == BITSERIAL_RESULT_SUCCESS){
            ...
            //DEBUG_LOGF("getReadSize = 0x%x 0x%x", value[0], value[1]);
        }
        #else
        result = BitserialWrite(handle,
                                BITSERIAL_NO_MSG,
...
int read_size = 0;
    ...
    DEBUG_LOGF("ReadOffSkin result= %d read %d ", result, read_size);
    ...
    if (BITSERIAL_RESULT_SUCCESS == getReadOffset(handle,offSkin_value, (uint16)read_size)){
        ...
        if(read_size>=2)
            DEBUG_LOGF("ReadOffSkin RxData[0] = 0x%x 0x%x",offSkin_value[0], offSkin_value[1]);
        if(read_size>=4)
            DEBUG_LOGF("ReadOffSkin RxData[2] = 0x%x 0x%x", offSkin_value[2], offSkin_value[3]);
        if(read_size >=6)
            DEBUG_LOGF("ReadOffSkin RxData[4] = 0x%x 0x%x",offSkin_value[4], offSkin_value[5]);
    }
...
static void HRMeasReading(bitserial_handle handle, uint8 *p_hr_meas, uint8 *p_length, uint8 sensor_status)
{
    ...
    bitserial_result result = BITSERIAL_RESULT_TIMEOUT;
    #ifdef Jim
    uint8 get_cmd[GET_CMD_LEN] = { 0x00, 0x05, PTEK_PKT_START_CHAR, 0x02, PTEK_CMD_GET, PTEK_HEART_RATE};
#else
    ...
    *p_hr_meas_flags = SENSOR_MEASUREVAL_FORMAT_UINT8 |sensor_status;
    ...
    /*  Read 0x7C */
    read_size = getReadSize(handle);
    DEBUG_LOGF("HRMeasReading result= %d read %d ", result, read_size);
    ...
    if (BITSERIAL_RESULT_SUCCESS == getReadOffset(handle,value,(uint16)read_size)){
        ...
        //if(read_size>=2)DEBUG_LOGF("HRMeasReading read_size = %d RxData[0] = 0x%x 0x%x",read_size, value[0], value[1]);
        //if(read_size>=4)DEBUG_LOGF("HRMeasReading RxData[2] = 0x%x 0x%x", value[2], value[3]);
        //if(read_size >=6)DEBUG_LOGF("HRMeasReading RxData[4] = 0x%x 0x%x",value[4], value[5]);
    }
    else
    {
...
//            MessageCancelFirst( sinkGetBleTask(), BLE_INTERNAL_MESSAGE_HR_READ_TIMER );
    ...
    if(p_hr_meas != NULL)
    {
        ...
        DEBUG_LOGF("1===GATT Heart Rate Measurement %d bpm\n", p_hr_meas[1]);
        /* Free the memory */
        free(p_hr_meas);
    }
    else
...
static void appBleNotifyHrMeasurements(GATT_HR_SERVER_TIMER_IND_T * ind)
{
...
#endif
    {
        ...
        DEBUG_LOGF("2===GATT Heart Rate Measurement %d bpm\n", p_hr_meas[1]);
        //free(p_hr_meas);
...
static bool ptekStartCmd(bitserial_handle handle)
{
    ...
    DEBUG_LOGF("ptekStartCmd result= %d size %d", result, read_size);
    ...
    if (BITSERIAL_RESULT_SUCCESS == getReadOffset(handle,value,(uint16)read_size)){
        if(read_size>=2)
            DEBUG_LOGF("start RxData[0] = 0x%x 0x%x", value[0], value[1]);
        if(read_size>=4)
            DEBUG_LOGF("start RxData[2] = 0x%x 0x%x", value[2], value[3]);
    }
    free(value);
...
static bool ptekStopCmd(bitserial_handle handle)
{
    ...
    /*  Read 0x7C */
    read_size = getReadSize(handle);
    DEBUG_LOGF("ptekStopCmd result= %d size %d", result, read_size);
    ...
    /*  Read Offset 0x7F */
    if (BITSERIAL_RESULT_SUCCESS == getReadOffset(handle,value,(uint16)read_size)){
        //if(read_size>=2)DEBUG_LOGF("Stop RxData[0] = 0x%x 0x%x", value[0], value[1]);
        //if(read_size>=4)DEBUG_LOGF("Stop RxData[2] = 0x%x 0x%x", value[2], value[3]);
    }
    free(value);
...
static bool ptekSetCmd(bitserial_handle handle)
{
    ...
    DEBUG_LOGF("ptekSetCmd result= %d size %d", result, read_size);
    ...
    /*  Read Offset 0x7F */
    if (BITSERIAL_RESULT_SUCCESS == getReadOffset(handle,value,(uint16)read_size)){
        //if(read_size>=2)DEBUG_LOGF("SetCmd RxData[0] = 0x%x 0x%x", value[0], value[1]);
        //if(read_size>=4)DEBUG_LOGF("SetCmd RxData[2] = 0x%x 0x%x", value[2], value[3]);
    }
    free(value);
...
static bool ptekGetCmd(bitserial_handle handle, uint8 command, uint8* value, uint8* read_size)
{
    bitserial_result result = BITSERIAL_RESULT_TIMEOUT;
#ifdef Jim
    uint8 get_cmd[GET_CMD_LEN] = { 0x00, 0x5, PTEK_PKT_START_CHAR, 0x02, PTEK_CMD_GET, command};
#else
    uint8 get_cmd[GET_CMD_LEN] = {PTEK_I2C_WRITE_ADDR, 0x00, 0x5, PTEK_PKT_START_CHAR, 0x02, PTEK_CMD_GET, command};
#endif
    result = BitserialWrite(handle,
                            BITSERIAL_NO_MSG,
                            get_cmd, sizeof(get_cmd),
    ...
    *read_size = getReadSize(handle);
    ...
    DEBUG_LOGF("ptekGetCmd CMD 0x%X size %d result= %d ", command, *read_size, result);

    if(*read_size==0) /* Read data must be at least 3 bytes. */
        return FALSE;

    if(!value)
        value = PanicUnlessMalloc(*read_size);

    /*  Read Offset 0x7F */
    if (BITSERIAL_RESULT_SUCCESS == getReadOffset(handle,value,(uint16)*read_size)){
        //if(*read_size>=2)DEBUG_LOGF("ptekGetCmd CMD 0x%X RxData[0] = 0x%x 0x%x",command, value[0], value[1]);
        //if(*read_size>=4)DEBUG_LOGF("ptekGetCmd CMD 0x%X RxData[2] = 0x%x 0x%x",command, value[2], value[3]);
        //if(*read_size >=6)DEBUG_LOGF("ptekGetCmd CMD 0x%X RxData[4] = 0x%x 0x%x",command, value[4], value[5]);
...
static bool checkPtekIdleMode(void)
{
    ...
    {
        //DEBUG_LOGF("checkPtekIdleMode read_size= %d",read_size);

        if(value)
        {
           // if(read_size>=2)
            //    DEBUG_LOGF("checkPtekIdleMode RxData[0] = 0x%x 0x%x", value[0], value[1]);
            //if(read_size>=4)
            //    DEBUG_LOGF("checkPtekIdleMode RxData[2] = 0x%x 0x%x", value[2], value[3]);
            if(read_size >=6)
            {
                //DEBUG_LOGF("checkPtekIdleMode RxData[4] = 0x%x 0x%x", value[4], value[5]);
                if(value[5]== 0)
                {
                    ret = TRUE;
                    //DEBUG_LOGF("checkPtekIdleMode ret TRUE = 0x%x",value[5]);
                }
                else
                {
            		//delay_ms(100);
                    //DEBUG_LOGF("checkPtekIdleMode ret FALSE = 0x%x",value[5]);
                }
            }
...
bool ptekGetIntOutPinStatus(const ptekConfig *config)
{
    ...
    if (PioCommonGetPio(config->pios.int_out) == TRUE) {
        DEBUG_LOGF("ptek INT OUT PIN = ON =======");
        ret = TRUE;
    }
    else {
        DEBUG_LOGF("ptek INT OUT PIN = OFF =======");
        ret = FALSE;
    }
...
bitserial_handle ptekEnable(const ptekConfig *config)
{
    ...
    DEBUG_LOGF("ptekEnable 0x%x",gPtekHandle);
...
void handleWriteHrMeasurementClientConfig(GATT_HR_SERVER_WRITE_CLIENT_CONFIG_IND_T * ind)
{
    ...
    */
    DEBUG_LOGF("GATT_HR_SERVER_WRITE_CLIENT_CONFIG_IND hrs=[0x%p] hr_server=[0x%p] value[0x%x]\n",
    ...
    if(ind->hr_sensor == &(pInfo->hr_server))
    {
        ...
        DEBUG_LOGF(" Heart rate Sensor client_config[0x%x]\n", pInfo->config.heart_rate);
...
static bool ptekGetCmd_Read(bitserial_handle handle, uint8 command, uint8* value, uint8* read_size)
{
    ...
    uint8 get_cmd[GET_CMD_LEN] = { 0x00, 0x4, PTEK_PKT_START_CHAR, 0x02, PTEK_CMD_GET, command};
...
static bool ReadOptical(void)//static void ReadOptical(void)
{
    ...
    if(ptekGetCmd_Read(gPtekHandle,(uint8)0x12,(uint8*)value,&read_size))
    {
        ...
        if(value)
        {
            //if(read_size>=2)DEBUG_LOGF("checkPtekIdleMode RxData[0] = 0x%x 0x%x", value[0], value[1]);
            //if(read_size>=4)DEBUG_LOGF("checkPtekIdleMode RxData[2] = 0x%x 0x%x", value[2], value[3]);
            if(read_size >=6)
            {
                //DEBUG_LOGF("checkPtekIdleMode RxData[4] = 0x%x 0x%x", value[4], value[5]);
                if(value[4] > 0x10)
                {
                    ret = TRUE;
                    //DEBUG_LOGF("checkPtekIdleMode ret TRUE = 0x%x",value[4]);
                }
                else
                {
                    //DEBUG_LOGF("checkPtekIdleMode ret FALSE = 0x%x",value[4]);
                }
            }
            //free(value); /* must free malloc after ptekGetCmd success */
        }
...
void PtekIR_Det(void)
{
    ...
    DEBUG_LOGF("Pre_Play ========= 0x%x",Pre_Play);
    DEBUG_LOGF("IR_play ========= 0x%x",IR_play);
    if(IR_play != Pre_Play)
    {
        ...
        if (appSmIsOutOfCase())
        {
            ...
            {	
                if (Pre_Play)
                {
					...
					{
						NotSendHR_Flag = 0;
                        //message->hr_sensor = 0x10900;     /* Pass the instance which can be returned in the response */
                        //message->cid = 0x101;                 /* Pass the CID so the client can be identified */
                        MessageSendLater(appGetGattHrsTask(), GATT_HR_SERVER_TIMER_IND,0, GATT_SERVER_HR_UPDATE_TIME);
					}
                ...
                {
					MessageSendLater(appGetUiTask(), Out_of_ear, NULL, D_SEC(10));
					if(appHfpIsCall()==FALSE)
                    {
                        appAvPause(FALSE);
                    }
