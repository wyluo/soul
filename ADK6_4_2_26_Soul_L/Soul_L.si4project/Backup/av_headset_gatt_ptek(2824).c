...
static void HRMeasReading(bitserial_handle handle, uint8 *p_hr_meas, uint8 *p_length, uint8 sensor_status)
{
    ...
    *p_hr_meas_flags = SENSOR_MEASUREVAL_FORMAT_UINT8 |sensor_status;
    ...
    if (BITSERIAL_RESULT_SUCCESS == getReadOffset(handle,value,(uint16)read_size)){
        ...
        if(read_size>=2)
            DEBUG_LOGF("HRMeasReading read_size = %d RxData[0] = 0x%x 0x%x",read_size, value[0], value[1]);
        if(read_size>=4)
            DEBUG_LOGF("HRMeasReading RxData[2] = 0x%x 0x%x", value[2], value[3]);
        if(read_size >=6)
            DEBUG_LOGF("HRMeasReading RxData[4] = 0x%x 0x%x",value[4], value[5]);
    }
    else
    {
