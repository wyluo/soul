...
#include "av_headset_scofwd.h"// add by jim
#include "av_headset_kymera_private.h"
#include "av_headset_sm_private.h"// add by jim
...
#ifdef SOUL_BLADE
    if(theInst->a2dp.disconnect_reason == AV_A2DP_DISCONNECT_LINK_KEY_MISSING)
    {
        if(appDeviceDelete(&theInst->bd_addr))
        {
            if (appSmIsOutOfCase())
        ...
        {
            MAKE_AV_MESSAGE(AV_INTERNAL_AUTO_PAIR_REQ);
            message->bd_addr = theInst->bd_addr;
            MessageCancelAll(&appGetAv()->task, AV_INTERNAL_AUTO_PAIR_REQ);
            MessageSendLater(&appGetAv()->task, AV_INTERNAL_AUTO_PAIR_REQ, message,100);
        }
    }
#endif  
}
...
static void appA2dpEnterConnectedSignalling(avInstanceTaskData *theInst)
{
    ...
    else
    {
		...
		if(PowerOff_Flag==0)
		{
			if(appConfigIsLeft())
			{
				appUiLeftChannel(theInst->a2dp.flags & A2DP_CONNECT_SILENT);
				MessageCancelAll(appGetSmTask(), SM_INTERNAL_Ptek_IR_Det);
				MessageSendLater(appGetSmTask(), SM_INTERNAL_Ptek_IR_Det, NULL, 1000);//JIM
			}
			else
				appUiRightChannel(theInst->a2dp.flags & A2DP_CONNECT_SILENT);
		}
		#endif
    }
        ...
        case A2DP_STATE_DISCONNECTED:
        {
            ...
            else
            {
                ...
                else
                {
					...
					/* add by jim 20190708 ====== reconnect failed */
					#ifdef AUTO_PAIRING  // jim 20190708
					...
					else
					{
			        	if (!appSmIsPairing())
						{
                            My_appScoDucoData_OTA(SFWD_OTA_Peer_PairingHeadset,1);
							appSmPairHandset();
						}
					}
					#endif
        ...
        case A2DP_STATE_CONNECTING_CROSSOVER:
        {
            ...
            else                
            {
                ...
                else
                {
                    ...
                    theInst->a2dp.disconnect_reason = AV_A2DP_CONNECT_FAILED;
                    #ifdef SOUL_BLADEx
                    if(cfm->status == a2dp_l2cap_key_missing)
                    {
                        theInst->a2dp.disconnect_reason = AV_A2DP_DISCONNECT_LINK_KEY_MISSING;
                    }
                    #endif
