...
#ifdef Jim
void my_appPowerDoPowerOff(void)
{
	appPowerDoPowerOff();
}
#endif
...
{
	#ifdef Jim
	if(ChargerStatus()==STANDBY) return TRUE;
	else
...
#ifdef Jim
 MessageSendConditionally(appGetPowerTask(), APP_POWER_INTERNAL_UI_COMPLETE,NULL, &appGetPower()->lock);
 #else
...
#ifdef Jim
bool my_appPowerOffRequest(void)
{
    DEBUG_LOG("PowerOffRequest  \n\n");
    ...
    if (appPowerCanPowerOff())  
    {
        switch (appGetPower()->state)
        {
            case POWER_STATE_INIT:
                ...
                break;
            case POWER_STATE_OK:
                appGetPower()->user_initiated_shutdown = TRUE;                //postponed to receive RX sync (appPeerSyncIsComplete()) 
                appPowerSetState(POWER_STATE_TERMINATING_CLIENTS_NOTIFIED);
                return TRUE;
            case POWER_STATE_TERMINATING_CLIENTS_NOTIFIED:
            case POWER_STATE_TERMINATING_CLIENTS_RESPONDED:
...
#ifdef SOUL_BLADE
bool appPowerOffRequestByPeer(void)
{
    DEBUG_LOG("appPowerOffRequestByPeer  \n\n");
    if (appPowerCanPowerOff())  // change the cocept to power off IN CASE
    {
        switch (appGetPower()->state)
        {
            case POWER_STATE_INIT:
    ...
    if (appPowerCanPowerOff())  
    {
        switch (appGetPower()->state)
        {
            ...
            case POWER_STATE_OK:
                ...
                {
                    peerSyncTaskData* ps = appGetPeerSync();        //   TX                    
                    ps->peer_poweroff_req = TRUE;
                    appConnRulesSetEvent(appGetSmTask(), RULE_EVENT_PEER_POWEROFF); 
                }
