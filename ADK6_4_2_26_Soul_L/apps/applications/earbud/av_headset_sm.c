 ...
 *****************************************************************************/
#ifdef Jim
My_hsTaskData My_theSink; // jim  20190630
...
static void appEnterInEarIdle(void)
{
    ...
    bool busy = appHfpIsCall()||appHfpIsScoActive()||appAvIsStreaming()||
        ps->peer_sco_active ||ps->peer_a2dp_streaming ||
        ps->peer_dfu_in_progress || appGetSubState() == APP_SUBSTATE_DFU || appGetSubState() == APP_SUBSTATE_DFU_CHECK;
...
#ifdef SOUL_BLADE
    MessageCancelAll( &appGetSm()->task, SM_STANDBY_PAIRING_TIMEOUT);

    if (appConfigStandbyTimeout())
    {
        DEBUG_LOGF("SM TIMER START 0x%x", appGetState());    
        MessageSendLater(&appGetSm()->task, SM_STANDBY_PAIRING_TIMEOUT, 0, D_SEC(appConfigStandbyTimeout()));
    }
#endif    
...
static void appExitInEarIdle(void)
{
	...
	MessageCancelAll(appGetUiTask(), appExitInEarIdle_LED);
#ifdef SOUL_BLADE
    MessageCancelAll( &appGetSm()->task, SM_STANDBY_PAIRING_TIMEOUT);
#endif    
}
...
#ifdef SOUL_BLADE
static void appSmHandleConnRulesPowerOff(void)
{
    DEBUG_LOG("appSmHandleConnRulesPowerOff");

    /* Send peer sync */
    appPowerOffRequestByPeer();

    /* In all cases mark rule as done */
    appConnRulesSetRuleComplete(CONN_RULES_PEER_POWEROFF);
}
#endif
...
static void appSmHandleConnRulesConnectHandset(CONN_RULES_CONNECT_HANDSET_T* crch)
{
    ...
    appPeerSyncSend(FALSE);
#ifdef Jim
	if(poweron_hold_enterpairing_flag == 0)
    {
...
{
//    if (appSmStateIsIdle(appGetState()))            //#ifdef SOUL_BLADE
...
void Save_PsKey_WriteTure(void)
{
    PsStore(199, &My_theSink.Data_1, sizeof(My_hsTaskData));// jim 20190703
}

void DUT_PsKey_WriteTure(void)
{
	My_theSink.DUT_Reboot = 1;
    PsStore(199, &My_theSink.Data_1, sizeof(My_hsTaskData));// jim 20190703

}

void my_PsKey_WriteTure(void)
{
	my_PsKey_Ready();
	My_theSink.Automatic_PowerOn = 1;
    PsStore(199, &My_theSink.Data_1, sizeof(My_hsTaskData));// jim 20190703

}

void my_PsKey_WriteFalse(void)
{
	My_theSink.Automatic_PowerOn = 0;
	PsStore(199, &My_theSink.Data_1, sizeof(My_hsTaskData));// jim 20190703
}

void my_PsKey_Ready(void)
{

	#define PSKEY__USR99 (199)
					
	PsRetrieve(PSKEY__USR99, &My_theSink.Data_1, sizeof(My_hsTaskData)); // jim  20190630

	DEBUG_LOGF("=============my_PsKey_Ready");
	DEBUG_LOGF(">> HS: My_theSink.Data_1= 0x%x",My_theSink.Data_1); // jim	20190630
	DEBUG_LOGF(">> HS: My_theSink.Automatic_PowerOn= 0x%x",My_theSink.Automatic_PowerOn);
	DEBUG_LOGF(">> HS: My_theSink.DUT_Reboot= 0x%x",My_theSink.DUT_Reboot);


}


#define pio0_get_level()           ((PioGet32Bank(0 / 32) & (1UL << 0)) ? 1 : 0) // jim 20190530
//extern void my_appPowerDoPowerOff(void);
...
static void appSmHandleNoDfu(void)
{
    ...
    if (appGetState() != APP_STATE_IN_CASE_DFU)
    {
        ...
        #ifdef LONG_PRESS_POWERON   // jim
        if(My_theSink.DUT_Reboot==1)
        {
			My_theSink.DUT_Reboot = 0;
			DUT_Flag = 1;
            PsStore(199, &My_theSink.Data_1, sizeof(My_hsTaskData));// jim 20190703
            appLedStopPattern(1);
            appLedSetFilter(app_led_filter_DUT, 1);
			MessageCancelAll(&appGetPairing()->task, PAIRING_INTERNAL_DISABLE_SCAN);
			ConnectionEnterDutMode();
		}
		else
		{
			if((My_theSink.Data_1 == 1)||(My_theSink.Automatic_PowerOn == 1)) // jim 20190703
	        {
	            My_theSink.Data_1 = 0;
	            PsStore(199, &My_theSink.Data_1, sizeof(My_hsTaskData));// jim 20190703
				appPowerOn();
	        ...
	        {
                DEBUG_LOGF("=================================appChargerIsConnected = %d", appChargerIsConnected());
                if(appChargerIsConnected()==CHARGER_DISCONNECTED)
	            {
					delay_ms(1000);
					if(pio0_get_level())
	                {
                        MessageSendLater(appGetSmTask(), SM_INTERNAL_TIMEOUT_LIMBO, NULL,15000);
	                }
	                else //debouce
	                {
...
static void appStandby_Pairing_Timeout_Poweroff(void)
{
    DEBUG_LOG("appStandby_Pairing_Timeout_Poweroff");
    peerSyncTaskData* ps = appGetPeerSync();
    
    bool busy = appDeviceIsHandsetConnected() || appHfpIsCall()||appHfpIsScoActive()||appAvIsStreaming() ||
        ps->peer_sco_active ||ps->peer_a2dp_streaming ||
        ps->peer_dfu_in_progress || appGetSubState() == APP_SUBSTATE_DFU || appGetSubState() == APP_SUBSTATE_DFU_CHECK;
    
    MessageCancelAll( &appGetSm()->task, SM_STANDBY_PAIRING_TIMEOUT);
    if (appGetState() == APP_STATE_IN_EAR_IDLE || !busy)
    {
		#ifdef Jim
		MessageSendLater(appGetUiTask(), APP_MFB_BUTTON_8_SECOND, NULL, 100);	
		#else
...
void appSmHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case CONN_RULES_SEND_PEER_SYNC:
            ...
            break;
#ifdef SOUL_BLADE            
        case CONN_RULES_PEER_POWEROFF:
            appSmHandleConnRulesPowerOff();
            break;
#endif            
		...
		case SM_INTERNAL_TIMEOUT_LIMBO:
		     MessageCancelAll(appGetSmTask(), SM_INTERNAL_TIMEOUT_LIMBO);//Alex
              //enterpoweroff_flag = 1;	// use for power off prompt
		     //appPowerOffRequest();
			 my_appPowerDoPowerOff();
		     break;
#endif
		case SM_INTERNAL_Ptek_IR_Det:
			PtekIR_Det();
			MessageCancelAll(appGetSmTask(), SM_INTERNAL_Ptek_IR_Det);
			MessageSendLater(appGetSmTask(), SM_INTERNAL_Ptek_IR_Det, NULL, 1000);
		break;


#ifdef SOUL_BLADE            
        case SM_STANDBY_PAIRING_TIMEOUT:
            appStandby_Pairing_Timeout_Poweroff();
            break;
#endif
        default:
...
#ifdef LONG_PRESS_POWERON
extern void appSmCancelLimboTimeout(void)
{
    MessageCancelAll(appGetSmTask(), SM_INTERNAL_TIMEOUT_LIMBO);
//	enterpoweroff_flag = 0;
}
#endif

#ifdef Jim
//add by kimny  jim
void appSmFactoryReset_later(uint32 delay)
{
    MessageSendLater(appGetSmTask(), SM_INTERNAL_FACTORY_RESET, NULL, delay);
}
#endif
