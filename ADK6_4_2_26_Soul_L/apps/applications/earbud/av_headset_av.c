...
#ifdef SOUL_BLADE
        case AV_INTERNAL_AUTO_PAIR_REQ:
        {
            AV_INTERNAL_AUTO_PAIR_REQ_T *req = (AV_INTERNAL_AUTO_PAIR_REQ_T *)message;
            MessageCancelAll(&theAv->task, AV_INTERNAL_AUTO_PAIR_REQ);
            if(appDeviceDelete(&req->bd_addr))
            {
                if (appSmIsOutOfCase())
                {
            ...
            {
                AV_INTERNAL_AUTO_PAIR_REQ_T *msg = PanicUnlessNew(AV_INTERNAL_AUTO_PAIR_REQ_T);
                msg->bd_addr = req->bd_addr;

                MessageSendLater(&theAv->task, AV_INTERNAL_AUTO_PAIR_REQ, msg,100);
            }
        }
            return;
#endif            
