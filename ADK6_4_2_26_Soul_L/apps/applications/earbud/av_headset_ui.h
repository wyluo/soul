...
#define APP_MFB_BUTTON_Slick					1110  // jim 20190625
#define APP_MFB_BUTTON_DOUBLE					1111
#define APP_MFB_BUTTON_TRIPLE					1112
#define APP_MFB_BUTTON_FOURTH					1113  // jim 20190625
#define OTA_Sync_POWER_OFF                      1114
#define UI_DoPowerOff                      		1115
#define APP_MFB_PANIC							1116
#define APP_MFB_Pairing							1117
#define APP_EVENT_RECONNECT_HEADSET				1118
#define APP_Battery_Critical                    1119
#define appExitInEarIdle_LED                    1120
#define Ptek_IR_Det			                    1121
#define Out_of_ear                           1122
...
extern const ledPattern app_led_pattern_calling[];
extern const ledPattern app_led_off[];
#endif
...
#define appUiHfpConnected(silent) \
    { if (!(silent)) {sinkStopAutoEnterPairingTimer();appUiPlayPrompt(PROMPT_CONNECTED);} } // jim 20190625
...
#define appUiHfpLinkLoss() \
    appUiPlayTone(app_tone_hfp_link_loss)  // jim  SLC link loss
...
#ifdef SOUL_BLADE
#define appUiCharging() \
    appLedSetPattern(app_led_pattern_charging, LED_PRI_EVENT)

#define appUiHfpCallingActive() \
    appLedSetPattern(app_led_pattern_calling, LED_PRI_HIGH)

#define appUiplayingMusic() \
    appLedSetPattern(app_led_off, LED_PRI_HIGH)

#define appUipowerOn() \
    appUiPlayTone(app_tone_power_on)

#define appUiDelete_Handsets() \
	appLedSetPattern(app_led_pattern_Delete_Handsets, LED_PRI_HIGH)  // jim 20190701

#define appUiDelete_TWS() \
		appLedSetPattern(app_led_pattern_Delete_TWS, LED_PRI_HIGH)  // jim 20190701


#ifdef INCLUDE_BME300_CES   
#define appUipowerOff() \
...
#define appUiAvConnected(silent) \
    { if (!(silent)) {/*appUiPlayPrompt(PROMPT_CONNECTED);*/sinkStopAutoEnterPairingTimer();}}// jim 20190625
...
/*! \brief Show AV streaming active LED pattern */ /* add by jim 20190309*/
#define appUiAvStreamingActive() \
	{ /*appLedEnable(TRUE);\
      appLedSetPattern(app_led_pattern_idle_connected, LED_PRI_MED);*/}
//(appLedSetPattern(app_led_pattern_streaming, LED_PRI_MED))  

/*! \brief Show AV APIX streaming active LED pattern */ /* add by jim 20190309*/
#define appUiAvStreamingActiveAptx() \
	{ /*appLedEnable(TRUE);\
      appLedSetPattern(app_led_pattern_idle_connected, LED_PRI_MED);*/}
    //( appLedSetPattern(app_led_pattern_streaming_aptx, LED_PRI_MED))

/*! \brief Cancel AV SBC/MP3 streaming active LED pattern */ /* add by jim 20190309*/
...
#define appUiIdleActive() \
    {appLedEnable(TRUE);appLedSetPattern(app_led_pattern_idle, LED_PRI_LOW);}

/*! \brief Show LED pattern for connected headset */
#define appUiIdleConnectedActive() \
	{ /*appLedEnable(TRUE);\
    appLedSetPattern(app_led_pattern_idle_connected, LED_PRI_MED);*/} /* ad by jim 20190309 */
...
#define appUiRightChannel(silent) { if (!(silent)) appUiPlayPrompt(PROMPT_right_channel);}
/*
#define appUiRightChannel() \
    appUiPlayPrompt(PROMPT_right_channel)
*/
#define appUiLeftChannel(silent) { if (!(silent)) appUiPlayPrompt(PROMPT_left_channel);}
/*
#define appUiLeftChannel() \
    appUiPlayPrompt(PROMPT_left_channel)
*/
#define appUiCallEnded() \
    appUiPlayPrompt(PROMPT_call_ended)
#define appUiCallRejected() \
    appUiPlayPrompt(PROMPT_call_rejected)
...
//extern void delay_ms(uint16 v_msec_16); // jim


extern void my_PsKey_WriteTure(void);
extern void DUT_PsKey_WriteTure(void);
extern void Save_PsKey_WriteTure(void);
extern void sinkStartAutoReconectHeadsetTimer(void);// jim 20190625
extern void sinkStopAutoReconectHeadsetTimer(void); //add by kimny jim

extern void sinkStopAutoEnterPairingTimer(void); //add by kimny jim
extern void sinkStartAutoEnterPairingTimer(void); //add by kimny jim
extern bool PowerOnReconnectTimeoutFlag;
extern bool poweron_hold_enterpairing_flag;
extern bool PowerOff_Flag;

extern void my_PsKey_WriteFalse(void);
extern void my_PsKey_Ready(void);
