...
static void appGattHandleGattManDisconnectInd(GATT_MANAGER_DISCONNECT_IND_T *ind)
{
    ...
    if (appGattDeleteInstanceByConnId(ind->cid))
    {
		...
		BleHR_Flag = 0;
		NotSendHR_Flag=0;
        MessageCancelAll(appGetGattHrsTask(), GATT_HR_SERVER_TIMER_IND);//ptekDisable((const ptekConfig *)appConfigPtek());
        #endif
...
{
    DEBUG_LOG("appGattHandleConnectionLibraryMessages id : 0x%x  0x%p", id, message);
    switch (id)
    {
