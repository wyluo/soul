    ...
    while (appTaskListIterateWithData(theLp->role_task_list, &task, &data))
    {
        if (data.sink == sink)
        {
            ...
            if (status == hci_success)
            {
                ...
                {
#ifdef iphone11_isse
					Sink tempA2dpSink = appAvGetSink(appAvGetA2dpSink(AV_CODEC_NON_TWS));
					Sink tempHfpSink = appHfpGetSink();
					if((data.sink != tempA2dpSink)&&(data.sink != tempHfpSink))
					{
					...
					{
						DEBUG_LOGF("appLinkPolicyUpdateRole, not run role-switch");
					}
#else
