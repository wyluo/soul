...
static void MY_appScoFwdPlayRingAtWcTime(rtime_t sync_time,uint8 my_ota_msg_id,uint8 my_duco_data)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();
    ...
    if (RtimeWallClockGetStateForSink(&wc_state, theScoFwd->wallclock_sink))
    {
        ...
        if (RtimeWallClock24ToLocal(&wc_state, sync_time, &local))
        {
            ...
            /* Wait for specified delay */
			DEBUG_LOGF(">> HS: msg[0]my_ota_msg_id = 0x%x",my_ota_msg_id);
			DEBUG_LOGF(">> HS: msg[0]my_duco_data = 0x%x",my_duco_data);
            /*
            {
                MessageSendLater(&theScoFwd->task, SFWD_INTERNAL_PLAY_RING, 0, delay);
            }
            */
            switch (my_ota_msg_id)
            {


				case SFWD_OTA_POWEROFF_TONE:
				MessageSendLater(appGetUiTask(),OTA_Sync_POWER_OFF,NULL,delay);
				break;
...
static void MYPEER_appScoFwdPlayRingAtWcTime(rtime_t sync_time,uint8 my_ota_msg_id,uint8 my_duco_data)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();
...
static void MY_appScoFwdHandleOTARing(const uint8* msg, int msg_size,uint8 my_ota_msg_id,uint8 my_duco_data)
{
    if(msg_size == 3)
    {
        rtime_t sync_time;
		DEBUG_LOGF(">> HS: msg[0] = 0x%x",msg[0]);
		DEBUG_LOGF(">> HS: msg[1] = 0x%x",msg[1]);
		DEBUG_LOGF(">> HS: msg[2] = 0x%x",msg[2]);
		DEBUG_LOGF(">> HS: msg[3] = 0x%x",msg[3]);
		DEBUG_LOGF(">> HS: msg[4] = 0x%x",msg[4]);
        sfwd_rx_help_read_ttp(msg, &sync_time);
		DEBUG_LOGF(">> HS: msg1[0] = 0x%x",msg[0]);
		DEBUG_LOGF(">> HS: msg1[1] = 0x%x",msg[1]);
		DEBUG_LOGF(">> HS: msg1[2] = 0x%x",msg[2]);
		DEBUG_LOGF(">> HS: msg1[3] = 0x%x",msg[3]);
		DEBUG_LOGF(">> HS: msg1[4] = 0x%x",msg[4]);
        MYPEER_appScoFwdPlayRingAtWcTime(sync_time,my_ota_msg_id,my_duco_data);
    }
}
...
static void ProcessOTAControlMessage(uint8 ota_msg_id, const uint8* payload, int payload_size)
{
    ...
    switch (ota_msg_id)
    {
        ...
        case SFWD_OTA_MSG_MICFWD_STOP:
            ...
            break;
		case SFWD_OTA_POWEROFF_TONE:
			MY_appScoFwdHandleOTARing(payload, (payload_size-2),SFWD_OTA_POWEROFF_TONE,payload[4]);

			break;

        case SFWD_OTA_Peer_PairingHeadset:
			PeerPairingFlag = payload[0];
			break;
...
/* Add by jim SBT 20191107 */
void My_appScoDucoData(uint8 duco_ota_msg_id,uint8 duco_buff)
{
#if 1
	scoFwdTaskData *theScoFwd = appGetScoFwd();
	rtime_t wallclock, sync_time;
		...
		RtimeLocalToWallClock24(&wc_state, VmGetTimerTime(), &wallclock))
	{
		...
		sync_time = rtime_add(wallclock, appConfigAvrcpMs() * 1000);
	
		sfwd_tx_help_write_ttp(buff,sync_time);
		DEBUG_LOGF(">> HS: buff[0] = 0x%x",buff[0]);
		DEBUG_LOGF(">> HS: buff[1] = 0x%x",buff[1]);
		DEBUG_LOGF(">> HS: buff[2] = 0x%x",buff[2]);
		duco_data[0] = buff[0];
		duco_data[1] = buff[1];
		duco_data[2] = buff[2];
		duco_data[3] = duco_ota_msg_id;
		duco_data[4] = duco_buff;
		/* Send the SCOFWD OTA message */
		DEBUG_LOGF(">> HS: msg[0]my_ota_msg_id = 0x%x",duco_ota_msg_id);
		DEBUG_LOGF(">> HS: msg[0]my_duco_data = 0x%x",duco_buff);
		SendOTAControlMessageWithPayload(duco_ota_msg_id, duco_data, sizeof(duco_data));
		//SendOTAControlMessageWithPayload(SFWD_OTA_MSG_RING, buff, sizeof(buff));
	
		/* Also, play the ring locally: for this we will use the same function that will
		   be called on the peer to handle the msg SFWD_OTA_MSG_RING */
		MY_appScoFwdPlayRingAtWcTime(sync_time,duco_ota_msg_id,duco_buff);
	...
	SendOTAControlMessageWithPayload(duco_ota_msg_id, &duco_buff, sizeof(duco_buff));
#endif
}

void My_appScoDucoData_OTA(uint8 duco_ota_msg_id,uint8 duco_buff)
{
	SendOTAControlMessageWithPayload(duco_ota_msg_id, &duco_buff, sizeof(duco_buff));
}
