...
#ifdef SOUL_BLADE     
    if (message->type == DM_HCI_READ_STORED_LINK_KEY_CFM)
    {
       PRINT(("=====MINI===== DM_HCI_READ_STORED_LINK_KEY_CFM state %d\n",state));
       connectionHandleReadStoredLinkKeyCfm(theCm->theAppTask,(const DM_HCI_READ_STORED_LINK_KEY_CFM_T *) message);
       return;
    }
#endif     
...
#ifdef SOUL_BLADE
        case CL_INTERNAL_DM_READ_STORED_LINK_KEY_REQ:
            PRINT(("CL_INTERNAL_DM_READ_STORED_LINK_KEY_REQ\n"));
            connectionHandleReadStoredLinkKeyReq((const CL_INTERNAL_DM_READ_STORED_LINK_KEY_REQ_T *)message);
            break;
#endif            
